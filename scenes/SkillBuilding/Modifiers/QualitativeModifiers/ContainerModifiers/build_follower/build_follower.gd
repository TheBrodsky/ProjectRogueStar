...
var _built_follower: PackedScene = null


func _ready() -> void:
	if _built_follower == null:
...
func modify_initialization(state: ActionState, container: EventContainer) -> void:
	if follower_of == FollowOf.ACTION:
		container.action_follower = _built_follower
		state.stats.follower.disable_rotation = disable_rotation
	else:
		container.container_follower = _built_follower
...
func _build_follower_scene() -> void:
	...
	_built_follower = ScenePacker.packScene(root_follower)
